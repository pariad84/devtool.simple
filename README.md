...
- **Grid** — a small spreadsheet-like table (name + a 3x3 grid of text cells to start). Click a cell to select it, drag to select a rectangular range, Ctrl+C/Ctrl+V copy and paste that range as tab/newline-separated text (Excel's clipboard format) — pasting a bigger block than the current grid grows it to fit, anchored at the top-left of your selection. Cell contents containing tabs or newlines aren't supported (no quote-escaping), same as a quick paste from Excel without special characters.
