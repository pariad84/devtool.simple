...
Memo, Sheet, Bookmark, Reminder, Capture, Code, Request, History, Resource, and Setting are all fixed in code, not stored as data — there's nothing to delete or heal, and none of them appear as a row in the Resource list. Resources you create yourself stay freely deletable, and are the only ones that show up there.

The ⚙ button in the DevTool popup's own header opens **Setting**: default popup scale/opacity (popups always size to their content), Resources (define your own data types — a name, storage key, and a JSON column spec; every resource opens as a list, and Columns can supply custom render functions for list cells or form fields, or reference another resource's rows by id) and Codes (lookup values like HTTP methods that populate select inputs elsewhere) for schema management, Generate sample data (repopulate the sample Memo/Bookmark/Request rows on demand), Export (download all data as JSON) / Import (restore from a JSON file, overwriting everything) for backups, and Reset (wipe all data back to the seeded samples).

A column's `form.type`/`list.type` can be `jsonobject` or `jsonarray` for a field that holds JSON text (like Request's Params or Capture's captured Data): a "View as Form"/"View as List" button next to it (or, in a list cell, a "View" button opening a popup) converts the raw JSON text to a structured form or table and back, without touching the underlying text as long as it's left untouched. In a record's own edit form, the `jsonobject` form view is editable (edits sync back live, so Save works whether you toggle back to JSON first or not); a list cell's popup view stays read-only, since it isn't wired to any save flow. `jsonarray`'s structured view is always read-only.

A column can also be `type: 'button'` (on `form` or `list`) instead of writing out a full `render` function: give it `text` and a `click` function-source (`function(data, e) { ... }`, `e` being the click event) and the layout builds the `action-btn` and wires up `e.stopPropagation()` for you.
