(function mini(global) {
    ...
    // including a resource-reference select (column.form.resource:{key,label}) and the render
    // escape hatch -- no per-resource form code needed.
    fn.component.layout.set({
        name : 'form',
...
if (typeof document !== 'undefined') {
    (function demo() {
        ...
        var itemResource = {
            ...
            columns : [
                ...
                { name : 'categoryId', label : 'Category', list : { type : 'text' }, form : { type : 'select', resource : { key : 'category', label : 'name' } } },
                {
                    ...
                    list : { render : 'function(data) { var b = document.createElement("button"); b.textContent = data.done ? "✓" : "✗"; b.addEventListener("click", function(e) { e.stopPropagation(); fn.data.update({ key: "item", id: data.id, data: { name: data.name, categoryId: data.categoryId, done: !data.done } }); e.target.closest(".__popup").refresh(); }); return b; }' },
                },
            ],
        };

        if (fn.data.select({ key : 'category' }).length === 0) {
            var groceries = fn.data.insert({ key : 'category', data : { name : 'Groceries' } });
            fn.data.insert({ key : 'category', data : { name : 'Chores' } });
            fn.data.insert({ key : 'item', data : { name : 'Milk', categoryId : groceries.id, done : false } });
            fn.data.insert({ key : 'item', data : { name : 'Vacuum', categoryId : groceries.id, done : true } });
        }
        ...
        fn.component._.openValue({ value : ['sample', 'shape-driven', 'dispatch'], title : 'openValue(array)' });
        fn.component._.openValue({ value : 'plain string value', title : 'openValue(string)' });
    })();
}
